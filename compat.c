...
static uint64_t rand_iter48_xsubi(unsigned short xsubi[3]) {
    ...
    xsubi[0] = (unsigned short)((x >> 32) & 0XFFFF);
    xsubi[1] = (unsigned short)((x >> 16) & 0XFFFF);
    xsubi[2] = (unsigned short)(x & 0XFFFF);

    return x;
}

#ifndef HAVE_NRAND48
uint64_t nrand48(unsigned short xsubi[3]) {
    // range [0, 2^31)
...
long jrand48(unsigned short xsubi[3]) {
    ...
    v.u = (uint32_t)(rand_iter48_xsubi(xsubi) >> 16); // C-style type casting
